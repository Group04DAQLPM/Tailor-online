<template>
  ...
  >
    ...
    <!-- Events Filters -->
    <div
      v-if="customizedOptions.filters.visibility && !shortcode.eventId"
    ...
    >
      ...
      <Transition name="am-slide-fade">
        ...
        >
          <div
            v-if="!shortcode.eventTag && tags.length > 0 && !licence.isLite"
          ...
          <div
            v-if="locations.length > 0 && !licence.isLite"
            class="am-els__filters-menu__items"
...
// * Tags array
let tags = computed(() => store.getters['eventEntities/getTags'])

// * Filter class generated according to filter elements visibility
let filterClassWidth = computed(() => {
  // p - preselected
  // a - array
  let pTag = shortcode.value.eventTag
  let aTag = tags.value.length
  let aLocation = locations.value.length

  let tagVisibility = !pTag && aTag > 0 && !licence.isLite
  let locationVisibility = aLocation > 0 && !licence.isLite
