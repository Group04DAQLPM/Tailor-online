    ...
    <div class="am-fs__payments-app-info">
      ...
      <div v-if="hasDeposit && paymentGateway !== 'onSite'">
        <div class="am-fs__payments-app-info-total">
          ...
          <span class="am-amount">
            {{ useFormattedPrice((paymentDeposit ? totalAmount : depositAmount)) }}
          </span>
        </div>
        ...
        <div class="am-fs__payments-app-info-total">
          ...
          <span class="am-amount">
            {{ useFormattedPrice(paymentDeposit ? 0 : (totalAmount - depositAmount)) }}
          </span>
        </div>
      </div>
...
function couponApplied () {
  ...
  emits(
    'setOnSitePayment',
      useAppointmentsAmount(store, service, appointments.prepaid) + useAppointmentExtrasAmount(store, service, appointments.prepaid) - discount.value.prepaid <= 0
  )
}
