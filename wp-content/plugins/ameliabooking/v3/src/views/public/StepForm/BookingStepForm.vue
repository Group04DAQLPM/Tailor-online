<template>
  ...
  >
    ...
    <MainContent>
      ...
      <template #footer>
        <MainContentFooter
          :second-button-show="stepsArray[stepIndex] === congratulationsStep && amSettings.roles.customerCabinet.enabled && amSettings.roles.customerCabinet.pageUrl !== null && !licence.isLite"
          :booked="booked"
          :loading="loading"
...
watch(ready, (current) => {
  if (current) {
    ...
    empty.value = store.getters['entities/getServices'].length === 0 ||
        ...
        (store.getters['entities/getPackages'].length === 0 && (shortcodeData.value.show === 'packages' || preselected.package))
...
function setShortcodeParams () {
  ...
  } else if (preselected.show === 'packages') {
...
function checkIfAllNotFree () {
  ...
  if (preselected.show === 'packages' || preselected.package) {
      ...
      let providers = employees.filter(eS => eS.serviceList.find(s => s.id === service.id && (s.price > 0 ||
        (s.customPricing && s.customPricing.enabled &&
          (Object.values(s.customPricing.durations).length === Object.values(s.customPricing.durations).filter(cp => cp.price > 0).length ||
            (store.getters['booking/getDuration'] && s.customPricing.durations[store.getters['booking/getDuration']].price > 0)
          )
        ))
      ))
...
let keepPaymentStep = computed(() => checkIfAllNotFree())
