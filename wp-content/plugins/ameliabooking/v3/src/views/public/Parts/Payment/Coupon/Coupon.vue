<template>
  ...
  >
    ...
    <AmInput
      ...
      :icon-end="code && (coupon.discount || coupon.deduction) ? (coupon.limit === 0 ? IconInfoReverse :  IconCheck) : ''"
...
let code = computed({
  get: () => store.getters['coupon/getCode'],
  set: (val) => {
  ...
  if (coupon.value.code) {
    httpClient.post(
      '/coupons/validate',
      {
        ...
        id: props.entityId,
        type: store.getters['bookableType/getType'],
        user: {
        ...
        count: props.bookingsCount
      }
    ).then(response => {
      store.commit('coupon/setCoupon', {
