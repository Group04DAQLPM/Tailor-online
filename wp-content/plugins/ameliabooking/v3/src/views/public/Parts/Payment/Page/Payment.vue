...
let availablePayments = computed(() => {
  if (ready.value) {
    ...
    let payments = !entityPayments ? {
      ...
      onSite: 'onSite' in entityPayments && entityPayments.onSite && amSettings.value.payments.onSite,
      stripe: 'stripe' in entityPayments && entityPayments.stripe && entityPayments.stripe.enabled && amSettings.value.payments.stripe.enabled,
      payPal: 'payPal' in entityPayments && entityPayments.payPal && entityPayments.payPal.enabled && amSettings.value.payments.payPal.enabled,
      wc: 'wc' in entityPayments && entityPayments.wc && entityPayments.wc.enabled && amSettings.value.payments.wc.enabled,
      mollie: 'mollie' in entityPayments && entityPayments.mollie && entityPayments.mollie.enabled && amSettings.value.payments.mollie.enabled,
      razorpay: 'razorpay' in entityPayments && entityPayments.razorpay && entityPayments.razorpay.enabled && amSettings.value.payments.razorpay.enabled,
    }
