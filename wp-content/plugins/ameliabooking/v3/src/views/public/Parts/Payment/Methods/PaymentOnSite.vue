<template>
  <div class="am-fs__payment_default">
    <div
      v-if="settings.general.googleRecaptcha.enabled"
...
function onSiteBooking (bookingData) {
  if (settings.value.general.googleRecaptcha.enabled && !settings.value.general.googleRecaptcha.invisible && !recaptchaValid.value) {
    emits('payment-error', amLabels.recaptcha_error)
  ...
  useCreateBooking(
    ...
    error => {
      ...
      useCreateBookingError(
        ...
        error.response.data,
        () => {
          if (settings.value.general.googleRecaptcha.enabled && settings.value.general.googleRecaptcha.invisible) {
...
function continueWithBooking () {
  ...
  if (settings.value.general.googleRecaptcha.enabled) {
    if (settings.value.general.googleRecaptcha.invisible) {
