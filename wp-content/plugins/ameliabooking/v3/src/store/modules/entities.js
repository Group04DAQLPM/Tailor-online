...
function setEntities ({ commit }, entities, types, licence) {
...
export default {
  ...
  mutations: {
    ...
    setPreselectedValues (state) {
      ...
      if (state.preselected.package) {
        state.packages = state.packages.filter(p => p.id ===  parseInt(state.preselected.package))
       }
    ...
    getEntities ({ commit }, payload) {
      ...
      if (payload.loadEntities && !getEntitiesVariableName()) {
        httpClient.get('/entities', { params: useUrlParams({ types }) }).then(response => {
          ...
          setEntities({ commit }, entities, types, payload.licence)
        })
      } else {
