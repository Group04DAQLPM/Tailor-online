...
function useBookingData (store, formData, mandatoryJson = false, paymentData = {}, recaptcha = null) {
    ...
    bookings: [
      {
        ...
        locale: locale,
        utcOffset: null,
        persons: bookableType !== 'event' ? store.getters['booking/getBookingPersons'] : store.getters['persons/getPersons'],
  ...
  switch (bookableType) {
    case ('appointment'):
      ...
      jsonData.bookings[0].extras = store.getters['booking/getSelectedExtras']
...
function useAppointmentCalendarData (store, response) {
  ...
  response.recurring.forEach((data) => {
    data.utcTime.forEach((date) => {
      ...
      if (response.bookable.recurringPayment > 0 && response.payment.gateway !== 'wc') {
...
function useEventCalendarData (store, response) {
  ...
  response.utcTime.forEach(function (date) {
    data.push(
      {
        ...
        start: settings.general.showClientTimeZone ? useLocalValue(date.start) : date.start,
        end: settings.general.showClientTimeZone ? useLocalValue(date.end) : date.end,
        utcStart: moment.utc(date.start.replace(/ /g, 'T')).toDate(),
        utcEnd: moment.utc(date.end.replace(/ /g, 'T')).toDate(),
      }
    )
