    ...
    <template #default>
      <div class="am-dialog-new-customize__content">
        <div class="am-dialog-new-customize__content-img">
          <img :src="`${$root.getUrl}v3/src/assets/img/admin/customize/amelia-event-list-2-0.png`" :alt="$root.labels.customize_dialog_heading">
        </div>
  ...
  methods: {
    goToCustomize () {
      this.$http.post(`${this.$root.getAjaxUrl}/settings`, {activation: {showAmeliaPromoCustomizePopup: false}})
        .then(response => {
          window.location.href = 'admin.php?page=wpamelia-customize-new&current=elf'
        })
        .catch(e => {
        })
    },
